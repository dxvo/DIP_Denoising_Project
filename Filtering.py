...
class Filter(tk.Tk):
    def __init__(self,path):
        ...
        self.image_name = split_path[1].split(".")[0]  # this should give image name noise/gaussian.pnb - gaussian
    ...
    def adaptive_median_recursive(self,slice_image,cur_value,fil_height,fil_width,max):
        ...
        windows_vert_start = int(1/2 * (max - fil_height))
        windows_horiz_start = int(1/2 * (max - fil_width))
        windows_vert_end = int(1/2 * (max + fil_height))
        windows_horiz_end = int(1/2 * (max + fil_width))
        ...
        if zmed > zmin and zmed < zmax:
            ...
            if (fil_height + 2) > max or (fil_width + 2) > max:  # check the limit of windows
                return zmed
            else:
    ...
    def get_hwo(self):
        ...
        if(self.filter_type == "contraharmonic" or self.filter_type == "trim"):
            ...
            self.order = float(self.my_entry_order.get().replace(',', ''))
    ...
    def get_filter_size(self):
        ...
        width = tk.Label(self.input_window, text="Width:")
